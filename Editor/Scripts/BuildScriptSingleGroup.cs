...
public static class BuildButtons
{
	...
	[MenuItem( "Assets/Build Map", true )]
	...
	{
		var selectedGuid = AssetDatabase.AssetPathToGUID( AssetDatabase.GetAssetPath( Selection.activeObject) );

		var contentUpdateSchema = LoadFromGuid<ContentUpdateGroupSchema>(AssetDatabase.FindAssets(
			"ExportSettings_ContentUpdate t:ContentUpdateGroupSchema" )[0]);
		var buildOnSchema = LoadFromGuid<BundledAssetGroupSchema>(AssetDatabase.FindAssets(
			"ExportSettings_ON t:BundledAssetGroupSchema" )[0]);
		var buildOffSchema = LoadFromGuid<BundledAssetGroupSchema>(AssetDatabase.FindAssets(
			"ExportSettings_OFF t:BundledAssetGroupSchema" )[0]);

		var groups = AssetDatabase.FindAssets( "t: AddressableAssetGroup" );
		foreach ( var g in groups )
		{
			var groupPath = AssetDatabase.GUIDToAssetPath( g );
			var groupAsset = AssetDatabase.LoadAssetAtPath<AddressableAssetGroup>( groupPath );
			groupAsset.Schemas.Clear();
			if ( g == selectedGuid )
			{
				...
				//	"LocalBuildPath" );
				var path =  $"{pathNoExtension}Exported~/UnityData/{EditorUserBuildSettings.activeBuildTarget}";
				...
				groupAsset.Settings.profileSettings.SetValue( groupAsset.Settings.activeProfileId,
					"LocalBuildPath", path );
				groupAsset.Schemas.Add( buildOnSchema );
			}
			else
			{
				groupAsset.Schemas.Add( buildOffSchema );
			}

			groupAsset.Schemas.Add( contentUpdateSchema );
			groupAsset.SetDirty( AddressableAssetSettings.ModificationEvent.GroupSchemaAdded, groupAsset, true );
			//{

			var schema = groupAsset.GetSchema<BundledAssetGroupSchema>();
			//if ( schema != null )
			//{
			//	Debug.Log( $"g: {g}" );
			//	Debug.Log( $"guid: {guid}" );
			//	Debug.Log( $"g == guid: {g == guid}" );
			//	//savedSchemasInBuild.Add( (schema, schema.IncludeInBuild) );
			//	if ( g == guid )
			//	{
		...
		foreach ( var builder in AddressableAssetSettingsDefaultObject.Settings.DataBuilders )
		{
		...
		if ( !builderFound )
		{
			throw new Exception( "Builder not found, make new one with Assets/Create/Addressables/ContentBuilders/BuildScriptSingleGroup and add it to AddressableAssetSettings Scriptable Object in your project" );
		}
...
//        string buildPath = aaContext.Settings.RemoteCatalogBuildPath.GetValue(aaContext.Settings);
...
//        CreateCatalogFiles( jsonText, builderInput, aaContext, buildPath );
...
//    internal bool CreateCatalogFiles( string jsonText, AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext, string path )
//    {
...
//        m_CatalogBuildPath = Path.Combine( path, builderInput.RuntimeCatalogFilename );
